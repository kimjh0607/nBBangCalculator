(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))a(i);new MutationObserver(i=>{for(const r of i)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&a(d)}).observe(document,{childList:!0,subtree:!0});function n(i){const r={};return i.integrity&&(r.integrity=i.integrity),i.referrerPolicy&&(r.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?r.credentials="include":i.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function a(i){if(i.ep)return;i.ep=!0;const r=n(i);fetch(i.href,r)}})();function O(e,t){const n={};e.forEach(o=>{n[o.id]=0});const a={};e.forEach(o=>{a[o.id]={},t.forEach(l=>{a[o.id][l.id]=null})});const i={};e.forEach(o=>{i[o.id]=[]}),t.forEach(o=>{const l=e.filter(u=>o.participantIds.includes(u.id));if(l.length===0)return;let c={};if(o.splitDrink){const u=o.foodAmount||0,f=o.drinkAmount||0,h=o.beverageAmount||0,m=o.drinkerIds||[],I=o.beveragerIds||[],_=o.noFoodIds||[],x=l.filter(k=>!_.includes(k.id)).length,A=l.filter(k=>m.includes(k.id)).length,R=l.filter(k=>I.includes(k.id)).length,S=x>0?u/x:0,L=A>0?f/A:0,T=R>0?h/R:0;l.forEach(k=>{let E=0;_.includes(k.id)||(E+=S),m.includes(k.id)?E+=L:I.includes(k.id)&&(E+=T),c[k.id]=E})}else{const u=o.totalAmount||0,f=l.length>0?u/l.length:0;l.forEach(h=>{c[h.id]=f})}Object.keys(c).forEach(u=>{c[u]=B(c[u])}),l.forEach(u=>{a[u.id][o.id]=c[u.id]});const p=o.splitDrink?(o.foodAmount||0)+(o.drinkAmount||0)+(o.beverageAmount||0):o.totalAmount||0;l.forEach(u=>{u.id===o.payerId?n[u.id]+=p-c[u.id]:n[u.id]-=c[u.id]})}),e.forEach(o=>{const l=t.filter(m=>m.splitDrink&&m.participantIds.includes(o.id));if(l.length===0)return;const c=[],p=[],u=[],f=[];l.forEach(m=>{const I=(m.drinkerIds||[]).includes(o.id),_=(m.beveragerIds||[]).includes(o.id),x=(m.noFoodIds||[]).includes(o.id);x&&!I&&!_?c.push(m):_?p.push(m):!I&&!_?u.push(m):I&&x&&f.push(m)});const h=(m,I,_)=>_?I:`${m.map(x=>x.name||`${x.id}차`).join("·")} ${I}`;c.length>0&&i[o.id].push(h(c,"참석만",c.length===l.length)),p.length>0&&i[o.id].push(h(p,"음료",p.length===l.length)),u.length>0&&i[o.id].push(h(u,"비음주",u.length===l.length)),f.length>0&&i[o.id].push(h(f,"음식X",f.length===l.length))});const r={};e.forEach(o=>{let l=0;t.forEach(c=>{const p=a[o.id][c.id];p!==null&&(l+=p)}),r[o.id]=l});const d=j(n,e);return{matrix:a,totals:r,notes:i,transfers:d}}function j(e,t){const n=[],a={};t.forEach(d=>{a[d.id]=d.name});let i=[],r=[];for(Object.keys(e).forEach(d=>{const o=B(e[d]);o>0?i.push({id:d,amount:o}):o<0&&r.push({id:d,amount:Math.abs(o)})}),i.sort((d,o)=>o.amount-d.amount),r.sort((d,o)=>o.amount-d.amount);i.length>0&&r.length>0;){const d=i[0],o=r[0],l=Math.min(d.amount,o.amount);l>0&&n.push({from:a[o.id],fromId:o.id,to:a[d.id],toId:d.id,amount:B(l)}),d.amount-=l,o.amount-=l,d.amount<=0&&i.shift(),o.amount<=0&&r.shift()}return n}function B(e){return Math.round(e/100)*100}function $(e){return e==null?"-":e.toLocaleString("ko-KR")+"원"}const F={"004":"국민","088":"신한","020":"우리","081":"하나","011":"농협","003":"기업","023":"SC제일","032":"부산","034":"광주","031":"대구","037":"전북","035":"제주","007":"수협","002":"산업","045":"새마을금고","048":"신협","089":"케이뱅크","090":"카카오뱅크","092":"토스뱅크"};function C(e,t,n,a={}){const{transfers:i}=n,r=[];return i.length===0?(r.push("✅ 모두 정산 완료!"),r.join(`
...
`)+e;if(navigator.share){try{await navigator.share({title:s.meetingName||"정산 결과",text:n})}catch(d){if(d?.name==="AbortError")return;y(`공유 실패 (${d?.name||"알 수 없음"}): 클립보드로 복사합니다`),navigator.clipboard.writeText(n).catch(()=>{})}return}const a=document.getElementById("share-result-btn"),i=a?.innerHTML;a&&(a.disabled=!0,a.innerHTML='<i data-lucide="loader-circle" style="width:16px;height:16px;animation:spin 1s linear infinite"></i> 준비 중...',lucide.createIcons({nodes:[a]}));let r=null;if(typeof html2canvas<"u"){const d=document.createElement("div");d.style.cssText="position:fixed;top:-9999px;left:-9999px;padding:24px;background:#0d1117;min-width:360px;";try{const o=document.getElementById("receipt-summary-section"),l=document.getElementById("result-section");if(await document.fonts.ready,o){const u=o.cloneNode(!0);u.style.animation="none",d.appendChild(u)}const c=l.cloneNode(!0);c.style.animation="none",d.appendChild(c),document.body.appendChild(d);const p=await html2canvas(d,{backgroundColor:"#0d1117",scale:2,useCORS:!0,logging:!1});r=await new Promise(u=>p.toBlob(u,"image/png"))}catch{}finally{d.parentNode&&document.body.removeChild(d)}}try{if(r)try{await navigator.clipboard.write([new ClipboardItem({"image/png":r})]),y("이미지가 클립보드에 복사됐어요! 채팅창에 붙여넣기하세요.")}catch{const o=URL.createObjectURL(r),l=document.createElement("a");l.href=o,l.download="정산결과.png",l.click(),URL.revokeObjectURL(o),y("이미지 저장됨! 채팅창에 파일로 첨부해주세요.")}else await navigator.clipboard.writeText(n),y("클립보드에 복사되었습니다!")}catch{navigator.clipboard.writeText(n).catch(()=>{}),y("클립보드에 복사되었습니다!")}finally{a&&i&&(a.disabled=!1,a.innerHTML=i,lucide.createIcons({nodes:[a]}))}}function b(){const e=document.getElementById("app");e.innerHTML=`
  ...
  `}function rt(e,t){const n=e.drinkerIds||[],a=e.beveragerIds||[],i=e.noFoodIds||[],r=s.participants.map(c=>{const p=e.participantIds.includes(c.id),u=n.includes(c.id),f=a.includes(c.id),h=u?"drinker":f?"beverage":"sober",m=u?"🍺":f?"🥤":"🚫",I=u?"음주":f?"음료":"없음";return`
      <label class="check-item ${p?"check-item--checked":""}"
        ...
        ${e.splitDrink&&p?`
          ...
          <span class="drink-toggle drink-toggle--${h}"
  ...
  `}function ut(){const{matrix:e,totals:t,notes:n}=s.result;let a='<table class="result-table"><thead><tr>';a+="<th>이름</th>",s.rounds.forEach(d=>{a+=`<th>${g(d.name)}</th>`}),a+="<th>총합</th></tr></thead><tbody>",s.participants.forEach(d=>{a+="<tr>",a+=`<td>${g(d.name)}</td>`,s.rounds.forEach(o=>{const l=e[d.id][o.id],c=o.payerId===d.id,p=vt(o,d.id);l===null?a+='<td class="not-participated">-</td>':c?a+=`<td class="amount payer-cell">${$(l)}<span class="payer-chip">결제</span>${p}</td>`:l===0?a+='<td><span class="attend-only-badge">참석만</span></td>':a+=`<td class="amount">${$(l)}${p}</td>`}),a+=`<td class="total-col amount">${$(t[d.id])}</td>`,a+="</tr>"}),a+="</tbody></table>";const{transfers:i}=s.result;let r="";return i.length===0?r='<div class="transfer-done"><i data-lucide="check-circle"></i> 모두 정산 완료!</div>':r='<div class="transfer-list">'+[...new Set(i.map(o=>o.toId))].map(o=>{const l=s.payerBankInfos[o]||{},c=w.find(m=>m.code===l.bankCode)?.name||"",p=c&&l.accountNumber,f=i.filter(m=>m.toId===o).map(m=>`
      ...
      `:"";return`<div class="transfer-item">${f}${h}</div>`}).join("")+"</div>",`
