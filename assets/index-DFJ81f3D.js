(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const i of document.querySelectorAll('link[rel="modulepreload"]'))n(i);new MutationObserver(i=>{for(const r of i)if(r.type==="childList")for(const c of r.addedNodes)c.tagName==="LINK"&&c.rel==="modulepreload"&&n(c)}).observe(document,{childList:!0,subtree:!0});function a(i){const r={};return i.integrity&&(r.integrity=i.integrity),i.referrerPolicy&&(r.referrerPolicy=i.referrerPolicy),i.crossOrigin==="use-credentials"?r.credentials="include":i.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function n(i){if(i.ep)return;i.ep=!0;const r=a(i);fetch(i.href,r)}})();function N(e,t){const a={};e.forEach(o=>{a[o.id]=0});const n={};e.forEach(o=>{n[o.id]={},t.forEach(p=>{n[o.id][p.id]=null})});const i={};e.forEach(o=>{i[o.id]=[]}),t.forEach(o=>{const p=e.filter(l=>o.participantIds.includes(l.id));if(p.length===0)return;let d={};if(o.splitDrink){const l=o.foodAmount||0,u=o.drinkAmount||0,f=o.beverageAmount||0,g=o.drinkerIds||[],I=o.beveragerIds||[],x=o.noFoodIds||[],_=p.filter($=>!x.includes($.id)).length,w=p.filter($=>g.includes($.id)).length,B=p.filter($=>I.includes($.id)).length,S=_>0?l/_:0,C=w>0?u/w:0,T=B>0?f/B:0;p.forEach($=>{let E=0;x.includes($.id)||(E+=S),g.includes($.id)?E+=C:I.includes($.id)&&(E+=T),d[$.id]=E})}else{const l=o.totalAmount||0,u=p.length>0?l/p.length:0;p.forEach(f=>{d[f.id]=u})}Object.keys(d).forEach(l=>{d[l]=A(d[l])}),p.forEach(l=>{n[l.id][o.id]=d[l.id]});const m=o.splitDrink?(o.foodAmount||0)+(o.drinkAmount||0)+(o.beverageAmount||0):o.totalAmount||0;p.forEach(l=>{l.id===o.payerId?a[l.id]+=m-d[l.id]:a[l.id]-=d[l.id]})}),e.forEach(o=>{const p=t.filter(g=>g.splitDrink&&g.participantIds.includes(o.id));if(p.length===0)return;const d=[],m=[],l=[],u=[];p.forEach(g=>{const I=(g.drinkerIds||[]).includes(o.id),x=(g.beveragerIds||[]).includes(o.id),_=(g.noFoodIds||[]).includes(o.id);_&&!I&&!x?d.push(g):x?m.push(g):!I&&!x?l.push(g):I&&_&&u.push(g)});const f=(g,I,x)=>x?I:`${g.map(_=>_.name||`${_.id}차`).join("·")} ${I}`;d.length>0&&i[o.id].push(f(d,"참석만",d.length===p.length)),m.length>0&&i[o.id].push(f(m,"음료",m.length===p.length)),l.length>0&&i[o.id].push(f(l,"비음주",l.length===p.length)),u.length>0&&i[o.id].push(f(u,"음식X",u.length===p.length))});const r={};e.forEach(o=>{let p=0;t.forEach(d=>{const m=n[o.id][d.id];m!==null&&(p+=m)}),r[o.id]=p});const c=O(a,e);return{matrix:n,totals:r,notes:i,transfers:c}}function O(e,t){const a=[],n={};t.forEach(c=>{n[c.id]=c.name});let i=[],r=[];for(Object.keys(e).forEach(c=>{const o=A(e[c]);o>0?i.push({id:c,amount:o}):o<0&&r.push({id:c,amount:Math.abs(o)})}),i.sort((c,o)=>o.amount-c.amount),r.sort((c,o)=>o.amount-c.amount);i.length>0&&r.length>0;){const c=i[0],o=r[0],p=Math.min(c.amount,o.amount);p>0&&a.push({from:n[o.id],fromId:o.id,to:n[c.id],toId:c.id,amount:A(p)}),c.amount-=p,o.amount-=p,c.amount<=0&&i.shift(),o.amount<=0&&r.shift()}return a}function A(e){return Math.round(e/100)*100}function y(e){return e==null?"-":e.toLocaleString("ko-KR")+"원"}function L(e,t,a,n={}){const{transfers:i}=a,r=[],c=t.filter(d=>(d.receiptItems||[]).some(m=>m.name||m.unitPrice>0));c.length>0&&(r.push("🧾 영수증 내역"),c.forEach(d=>{const m=(d.receiptItems||[]).filter(u=>u.name||u.unitPrice>0);r.push(`
📍 ${d.name}`),m.forEach(u=>{const f=(u.quantity||0)*(u.unitPrice||0);r.push(`  ${u.name||"(항목)"} × ${u.quantity||0} = ${y(f)}`)});const l=m.reduce((u,f)=>u+(f.quantity||0)*(f.unitPrice||0),0);l>0&&r.push(`  소계: ${y(l)}`)}),r.push(""),r.push("─────────────────"),r.push("")),r.push("🧾 차수별 내역");let o=0;return t.forEach(d=>{const m=d.participantIds.length,l=d.splitDrink?(d.foodAmount||0)+(d.drinkAmount||0)+(d.beverageAmount||0):d.totalAmount||0;if(l===0)return;o+=l;const u=e.find(g=>g.id===d.payerId),f=u?` · ${u.name} 결제`:"";r.push(`${d.name}: ${y(l)} (${m}명${f})`)}),t.length>1&&r.push(`총 합계: ${y(o)}`),r.push(""),r.push("─────────────────"),i.length===0?(r.push("✅ 모두 정산 완료!"),r.join(`
...
`)+e,n=document.getElementById("share-result-btn"),i=n?.innerHTML;n&&(n.disabled=!0,n.innerHTML='<i data-lucide="loader-circle" style="width:16px;height:16px;animation:spin 1s linear infinite"></i> 준비 중...',lucide.createIcons({nodes:[n]}));try{const r=document.getElementById("receipt-summary-section"),c=document.getElementById("result-section");await document.fonts.ready;const o=document.createElement("div");if(o.style.cssText="position:fixed;top:-9999px;left:-9999px;padding:24px;background:#0d1117;min-width:360px;",r){const u=r.cloneNode(!0);u.style.animation="none",o.appendChild(u)}const p=c.cloneNode(!0);p.style.animation="none",o.appendChild(p),document.body.appendChild(o);const d=await html2canvas(o,{backgroundColor:"#0d1117",scale:2,useCORS:!0,logging:!1});document.body.removeChild(o);const m=await new Promise(u=>d.toBlob(u,"image/png")),l=new File([m],"정산결과.png",{type:"image/png"});if(navigator.canShare&&navigator.canShare({files:[l]}))await navigator.share({files:[l],text:a});else if(navigator.share)await navigator.share({text:a});else{const u=URL.createObjectURL(m),f=document.createElement("a");f.href=u,f.download="정산결과.png",f.click(),URL.revokeObjectURL(u),navigator.clipboard.writeText(a).catch(()=>{}),k("이미지 저장 + 텍스트 복사 완료!")}}catch(r){r?.name!=="AbortError"&&(navigator.share?navigator.share({text:a}).catch(()=>{}):(navigator.clipboard.writeText(a).catch(()=>{}),k("클립보드에 복사되었습니다!")))}finally{n&&i&&(n.disabled=!1,n.innerHTML=i,lucide.createIcons({nodes:[n]}))}}function b(){const e=document.getElementById("app");e.innerHTML=`
  ...
  `}function st(e,t){const a=e.drinkerIds||[],n=e.beveragerIds||[],i=e.noFoodIds||[],r=s.participants.map(d=>{const m=e.participantIds.includes(d.id),l=a.includes(d.id),u=n.includes(d.id),f=l?"drinker":u?"beverage":"sober",g=l?"🍺":u?"🥤":"🚫",I=l?"음주":u?"음료":"없음";return`
      <label class="check-item ${m?"check-item--checked":""}"
        ...
        ${e.splitDrink&&m?`
          ...
          <span class="drink-toggle drink-toggle--${f}"
